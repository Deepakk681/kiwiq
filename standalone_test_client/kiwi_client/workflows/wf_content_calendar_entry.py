...
async def main_test_content_calendar_workflow():
    ...
    # Example Inputs 
    test_entity_username = "test-user-123"
    test_context_docs = [
        {
            "filename_config": {
    ...
    setup_docs: List[SetupDocInfo] = [
        ...
        # User Preferences Document
        {
            ...
            'initial_data': {
                ...
                "post_length_preferences": {
                    "short": {"min": 100, "max": 300},
                    "medium": {"min": 300, "max": 600},
                    "long": {"min": 600, "max": 1200}
                },
                "hashtag_preferences": {
                    "count_per_post": {"min": 3, "max": 5},
                    "favorite_hashtags": ["#LeadershipInsights", "#DigitalTransformation", "#MarketingStrategy"]
                }
            }, 
        ...
        # Content Strategy Document
        {
            ...
            'initial_data': {
                ...
                "target_audience": {
                    "primary": "Senior marketing professionals",
                    "secondary": "Aspiring marketing leaders"
                },
                "voice_and_tone": {
                    "voice_characteristics": ["Authoritative", "Insightful", "Approachable"],
                    "tone_by_pillar": {
                        "Educational": "Informative and analytical",
                        "Inspirational": "Motivational and forward-looking",
                        "Conversational": "Relatable and authentic"
                    }
                },
                "content_objectives": [
                    "Position as thought leader in digital marketing",
                    "Build community of marketing professionals",
                    "Drive engagement through valuable insights",
                    "Support business growth through network expansion"
                ]
            }, 
    ...
    async def validate_calendar_output(outputs: Optional[Dict[str, Any]]) -> bool:
        ...
        posts_per_week = user_prefs.get('posting_schedule', {}).get('posts_per_week', 3) if user_prefs else 3
        ...
        for brief in outputs.get('final_briefs_list', []):
            ...
            # Validate that scheduled_date is a valid date
            try:
                # Handle date format - could be string or already date object
                if isinstance(brief['scheduled_date'], str):
                    datetime.strptime(brief['scheduled_date'], '%Y-%m-%d')
                elif not isinstance(brief['scheduled_date'], (date, datetime)):
                    raise ValueError("scheduled_date is not a valid date format")
            except ValueError:
                assert False, f"Invalid scheduled_date format: {brief['scheduled_date']}"
