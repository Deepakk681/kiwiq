...
2. Using map_list_router_node to route each item directly to parallel LLM processing branches
3. LLM nodes operating in private mode with passthrough data configuration
4. Using private_output_passthrough_data_to_central_state_key to preserve ID and name fields
5. Collecting all outputs in graph state with format: {output: text_content, id: original_id, name: description}
6. Validating that all items were processed correctly with passthrough data intact
...
workflow_graph_schema = {
    "nodes": {
        ...
        # --- 2. Map List Router Node - Routes each item to parallel LLM processing ---
        "route_items_to_llm": {
                ...
                "map_targets": [
                    {
                        ...
                        "batch_size": 1,
                        # "batch_field_name": "item_batch"
                        # Note: No batch_size specified, so each item goes to a separate branch
                    }
                ]
            }
        },

        # --- 3. LLM Node - Processes individual items in private mode ---
        "process_individual_item": {
            ...
            "private_output_passthrough_data_to_central_state_keys": ["id", "name", "description"],  # Pass through ID and name
            ...
            "node_config": {
                ...
                # "user_prompt": "Please provide a comprehensive and helpful response to the following prompt:\n\n{user_prompt}"
                # Note: No structured output schema - will use text_content field
            }
        },

        # --- 4. Output Node with Fan-In ---
        "output_node": {
            "node_id": "output_node",
    ...
    "edges": [
        ...
        # Router to LLM (private mode) - each item goes directly to LLM
        {"src_node_id": "route_items_to_llm", "dst_node_id": "process_individual_item", "mappings": []},
