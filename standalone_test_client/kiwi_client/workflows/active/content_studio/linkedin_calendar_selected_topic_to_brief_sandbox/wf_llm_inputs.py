...
# 1. Brief Generation - Generate comprehensive LinkedIn content brief from selected topic
# 2. Feedback Processing - Process HITL feedback and iterate on brief
...
# Additional schemas needed for BriefGenerationOutput
...
class StructureOutlineSchema(BaseModel):
    ...
    evidence_strength_reasoning: str = Field(
        description="Why this evidence effectively supports your perspective"
    )

    unique_angle: str = Field(description="Unique angle or approach")

    angle_differentiation_reasoning: str = Field(
        description="How this angle differs from typical industry discussions"
    )

    call_to_action: str = Field(description="Call to action or conclusion")

    cta_effectiveness_reasoning: str = Field(
        description="Why this CTA will drive desired audience behavior"
    )
...
class ContentBriefSchema(BaseModel):
    ...
    engagement_strategy_reasoning: str = Field(
        description="How this content approach will maximize engagement on LinkedIn"
    )

    viral_potential_elements: List[str] = Field(
        description="Specific elements designed to increase shareability and reach"
    )
...
# =============================================================================
# Generates a comprehensive LinkedIn content brief from a selected topic
...
# =============================================================================
# Processes user feedback and provides revision instructions
...
# User prompt template for brief generation
# Variables used:
# - {selected_topic}: The topic selected by the user
# - {content_calendar_context}: Context from the content calendar
# - {executive_profile}: Executive's profile information
# - {content_strategy}: Content strategy document
# - {scheduled_date}: Date when content will be published
...
# Initial user prompt for feedback analysis
# Variables used:
# - {original_brief}: The original brief generated
# - {user_edits}: User's edits to the brief
...
# Additional user prompt for continued feedback
# Variables used:
# - {updated_brief}: The updated brief after previous feedback
# - {user_edits}: New user edits
# - {user_feedback}: New user feedback
# - {revision_history}: History of previous revisions
