...
from kiwi_client.workflows.active.content_studio.linkedin_user_input_to_brief_sandbox.wf_linkedin_user_input_to_brief_json import (
    workflow_graph_schema,
)

from kiwi_client.workflows.active.content_studio.linkedin_user_input_to_brief_sandbox.wf_testing.wf_inputs import (
    test_name,
    test_scenario,
)

from kiwi_client.workflows.active.content_studio.linkedin_user_input_to_brief_sandbox.wf_testing.sandbox_setup_docs import (
    setup_docs,
    cleanup_docs,
)

from kiwi_client.workflows.active.content_studio.linkedin_user_input_to_brief_sandbox.wf_testing.wf_run_hitl_inputs import (
    hitl_inputs,
)

WORKFLOW_NAME_TO_INGEST_AS_FOR_TESTING = "test_linkedin_user_input_to_brief_workflow"
...
def get_workflow_paths():
    ...
    workflow_folder = current_file_dir.parent  # workflow main folder (linkedin_user_input_to_brief_sandbox)
...
async def validate_linkedin_brief_output(outputs: Optional[Dict[str, Any]]) -> bool:
    """
    Validate the LinkedIn brief generation workflow outputs.
    ...
    logger.info("Validating LinkedIn user input to brief workflow outputs...")
    
    # Check for expected keys - brief generation workflow outputs
    expected_keys = ['transformed_data']
    for key in expected_keys:
        assert key in outputs, f"Validation Failed: '{key}' key missing."
    
    # Validate transformed data structure
    transformed_data = outputs.get('transformed_data', {})
    assert isinstance(transformed_data, dict), "Validation Failed: transformed_data should be a dict."

    # Check if document was successfully created/saved
    document_id = transformed_data.get('document_id')
    if document_id:
        logger.info("✓ LinkedIn content brief was successfully saved")
        logger.info(f"   Brief saved with document ID: {document_id}")
    else:
        logger.warning("⚠ No document ID found in outputs - brief may not have been saved")
    
    logger.info("✓ Output validation passed.")
    logger.info(f"   Document ID: {document_id if document_id else 'N/A'}")
    
    return True


async def main_test_linkedin_brief_generation():
    """
    Test the LinkedIn Brief Generation Workflow with enhanced functionality.
    """
    ...
    try:
        ...
        final_status, final_outputs = await run_workflow_test(
            ...
            validate_output_func=validate_linkedin_brief_output,
        ...
        if final_outputs:
            print(f"\nTest Results:")
            transformed_data = final_outputs.get('transformed_data', {})
            document_id = transformed_data.get('document_id')
            print(f"Document ID: {document_id if document_id else 'N/A'}")

            if document_id:
                print("✓ LinkedIn content brief was successfully saved")
                print(f"Brief saved with document ID: {document_id}")
            else:
                print("⚠ No document ID found - brief may not have been saved")
...
if __name__ == "__main__":
    print("="*60)
    print("LinkedIn Brief Generation Workflow Test")
    print("="*60)
    
    try:
        asyncio.run(main_test_linkedin_brief_generation())
    ...
    print("Run from project root: PYTHONPATH=. python kiwi_client/workflows/active/content_studio/linkedin_user_input_to_brief_sandbox/wf_testing/wf_runner.py")
