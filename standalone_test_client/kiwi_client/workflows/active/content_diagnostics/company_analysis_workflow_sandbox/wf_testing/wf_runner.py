...
from kiwi_client.workflows.active.content_diagnostics.company_analysis_workflow_sandbox.wf_company_analysis_workflow_json import (
    workflow_graph_schema,
)

from kiwi_client.workflows.active.content_diagnostics.company_analysis_workflow_sandbox.wf_testing.wf_inputs import (
    test_name,
    test_scenario,
)

from kiwi_client.workflows.active.content_diagnostics.company_analysis_workflow_sandbox.wf_testing.sandbox_setup_docs import (
    setup_docs,
    cleanup_docs,
)

WORKFLOW_NAME_TO_INGEST_AS_FOR_TESTING = "test_company_analysis_workflow_sandbox_workflow"
...
def get_workflow_paths():
    ...
    workflow_folder = current_file_dir.parent  # workflow main folder (company_analysis_workflow_sandbox)
...
async def validate_company_analysis_workflow_output(outputs: Optional[Dict[str, Any]]) -> bool:
    """
...
async def main_test_company_analysis_workflow():
    """
    ...
    try:
        final_status, final_outputs = await run_workflow_test(
            ...
            validate_output_func=validate_company_analysis_workflow_output,
...
if __name__ == "__main__":
    print("="*60)
    print("Company Analysis Workflow Test")
    print("="*60)
    
    try:
        asyncio.run(main_test_company_analysis_workflow())
    ...
    print("Run from project root: PYTHONPATH=. python kiwi_client/workflows_for_blog_teammate/wf_company_analysis_workflow.py")
