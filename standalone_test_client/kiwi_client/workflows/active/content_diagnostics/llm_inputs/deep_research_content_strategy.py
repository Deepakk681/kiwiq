...
# ───────────────────────────────────────────────────────────────────────────────
# STRUCTURED EXTRACTION PROMPTS FOR COMBINED DEEP RESEARCH
# ───────────────────────────────────────────────────────────────────────────────

# SYSTEM_PROMPT_TEMPLATE_FOR_STRUCTURED_EXTRACTION = """You are a strategic intelligence extraction specialist. Your task is to take unstructured research text and extract STRATEGIC INTELLIGENCE (not just data) into a perfectly structured JSON format.

# **PROGRESSIVE EXTRACTION MANDATE:**

# **Phase 1: Identify Critical Patterns**
# Extract the most impactful strategic patterns from the research text.

# **Phase 2: Analyze Key Mechanisms**
# For top patterns only, extract:
# - Core mechanism (why it works)
# - Competitive advantage potential  
# - Execution requirements

# **Phase 3: Structure Strategic Intelligence**
# Organize insights into the required JSON schema focusing on actionable intelligence.

# **CRITICAL REQUIREMENTS:**
# 1. **Strategic Intelligence Focus**: Prioritize insights that explain WHY and HOW over descriptive data
# 2. **Competitive Advantage Extraction**: Extract information that reveals competitive moats and differentiation
# 3. **Mechanism Analysis**: Capture explanations of psychological, behavioral, and market mechanisms
# 4. **Execution Intelligence**: Extract resource requirements, complexity assessments, and implementation barriers
# 5. **Context Dependency**: Capture market conditions where strategies work/fail
# 6. **Risk Assessment**: Extract potential pitfalls and mitigation strategies

# **EXTRACTION PRIORITIES:**
# - Strategic reasoning and mechanism analysis over raw percentages
# - Competitive differentiation insights over generic best practices  
# - Execution complexity and barriers over simple recommendations
# - Market context dependencies over universal truths
# - Performance evidence and validation over assumptions

# Extract strategic intelligence that drives competitive advantage, not just organized data."""

# USER_PROMPT_TEMPLATE_FOR_STRUCTURED_EXTRACTION = """**EXTRACTION TASK:**
# Below is unstructured research text that contains both content strategy analysis and LinkedIn research data. Extract and structure ALL information according to the provided JSON schema.

# **SOURCE TEXT:**
# {combined_text}

# **EXTRACTION INSTRUCTIONS:**
# - Carefully map each insight, fact, or recommendation from the source text to the most relevant field in the schema.
# - If information is missing for a required field, leave it empty or use null, but do not fabricate data.
# - For nested lists or objects, ensure each entry is complete and follows the schema structure.
# - When extracting qualitative insights (e.g., "competitive advantage potential"), paraphrase concisely and focus on the underlying mechanism or rationale.
# - For any ambiguous or multi-faceted information, split it into multiple schema entries as appropriate.
# - Maintain the logical flow and hierarchy of the schema, ensuring all parent and child fields are properly populated.


# **DATA PROCESSING RULES:**
# - Give percentages out of 100 (e.g., "25%" → 25)
# - Extract numerical scores and ratings exactly as presented (ensure all scores are on 1-100 scale)
# - Preserve all statistical data and performance metrics
# - Group similar content under appropriate schema fields
# - Ensure all nested objects and arrays are properly populated

# **OUTPUT REQUIREMENT:**
# Return ONLY the structured JSON object - no explanations, no markdown formatting, no additional text.

# Begin extraction now:"""

# ───────────────────────────────────────────────────────────────────────────────
# BLOG CONTENT STRATEGY EXTRACTION PROMPTS
# ───────────────────────────────────────────────────────────────────────────────
...
# ───────────────────────────────────────────────────────────────────────────────
# LINKEDIN STRATEGY EXTRACTION PROMPTS
# ───────────────────────────────────────────────────────────────────────────────
...
# ───────────────────────────────────────────────────────────────────────────────
# ENHANCED RESEARCH PROMPTS WITH COMPREHENSIVE DATA GATHERING INSTRUCTIONS
# ───────────────────────────────────────────────────────────────────────────────
