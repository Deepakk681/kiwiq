...
from kiwi_client.workflows.active.playbook.linkedin_content_playbook_generation.wf_linkedin_content_playbook_generation_json import (
    workflow_graph_schema,
)

from kiwi_client.workflows.active.playbook.linkedin_content_playbook_generation.wf_testing.wf_inputs import (
    test_name,
    test_scenario,
)
...
from kiwi_client.workflows.active.playbook.linkedin_content_playbook_generation.wf_testing.wf_validation import (
    validate_playbook_output,
)

from kiwi_client.workflows.active.playbook.linkedin_content_playbook_generation.wf_testing.wf_state_filter_mapping import (
    state_filter_mapping,
)

WORKFLOW_NAME_TO_INGEST_AS_FOR_TESTING = "test_linkedin_content_playbook_generation_workflow"
...
def get_workflow_paths():
    ...
    workflow_folder = current_file_dir.parent  # workflow main folder (blog_brief_to_blog_sandbox)
...
async def main_test_linkedin_playbook():
    """
    Test the LinkedIn Content Playbook Generation Workflow with enhanced functionality.
    """
    ...
    try:
        final_status, final_outputs = await run_workflow_test(
            ...
            hitl_inputs=[],  # HITL inputs provided interactively
        ...
        if final_outputs:
            print(f"\nTest Results:")
            print(f"✓ LinkedIn content playbook generated successfully")
...
if __name__ == "__main__":
    ...
    try:
        asyncio.run(main_test_linkedin_playbook())
