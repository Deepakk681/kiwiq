"""
Validation functions for Blog Content Playbook Generation workflow.
"""
...
async def validate_playbook_output(outputs: Optional[Dict[str, Any]]) -> bool:
    """
    Validate the Blog Content Playbook Generation workflow outputs.
    ...
    logger.info("Validating blog content playbook workflow outputs...")

    # Check for expected keys
    assert 'final_paths_processed' in outputs or outputs, "Validation Failed: No final outputs found."

    logger.info(" Blog content playbook workflow validation passed.")
