...
def main() -> None:
    ...
    try:
        ...
        st.caption(f"📄 **File Path:** `wf_llm_inputs.py`")
    ...
    if mode == "Variable-by-variable":
        ...
        if st.button("Update", key=f"btn_llm_{var_name}"):
            ...
            if ok:
                ...
                # If the full-file editor is open in another tab, refresh its session buffer from disk
                try:
                    meta = workflow.get("metadata", {})
                    category = meta.get("category")
                    workflow_name = meta.get("workflow_name")
                    ...
                    dirty_key = f"llm_full_content_dirty__{category}__{workflow_name}"
                    # Only overwrite editor buffer if user has no unsaved edits
                    if not st.session_state.get(dirty_key, False):
                        st.session_state[content_key] = latest
                except Exception:
                    pass
                st.rerun()
            else:
        ...
        if use_syntax_highlight:
            ...
            if response.get("id") and new_content != st.session_state[content_key]:
                ...
                # Mark buffer as dirty so background variable edits won't clobber unsaved changes
                st.session_state[f"llm_full_content_dirty__{category}__{workflow_name}"] = True
                st.rerun()
        else:
            # Plain text editor
        ...
        if st.button("Save LLM file", type="primary"):
            try:
                ...
                st.session_state[f"llm_full_content_dirty__{category}__{workflow_name}"] = False
                st.rerun()
            except Exception as e:
