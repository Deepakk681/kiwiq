...
def _render_json_with_limited_depth(data: Dict[str, Any], max_depth: int = 1) -> None:
    """Render JSON data with limited expansion depth to avoid cluttering the UI."""
    # Use st.json's built-in expanded parameter to control initial expansion depth
    # This keeps the full data intact while limiting visual clutter
    st.json(data, expanded=max_depth)
...
def main() -> None:
    ...
    # Render chat history
    # Count state messages to determine which ones to collapse
    state_messages = [i for i, msg in enumerate(st.session_state[chat_key]) if msg.get("type") == "state"]
    ...
    for i, msg in enumerate(st.session_state[chat_key]):
        ...
        with st.chat_message("user" if role == "user" else "assistant"):
            ...
            if mtype == "inputs":
                ...
                # Determine if this is one of the latest 2 state messages
                state_index = state_messages.index(i)
                is_recent_state = state_index >= len(state_messages) - 2
                
                content = msg.get("content", {})
                if is_recent_state:
                    # Show recent states expanded but with limited depth (max 1 level)
                    _render_json_with_limited_depth(content, max_depth=1)
                else:
                    # Collapse older states
                    with st.expander(f"State Snapshot {state_index + 1} (collapsed)", expanded=False):
                        _render_json_with_limited_depth(content, max_depth=1)
