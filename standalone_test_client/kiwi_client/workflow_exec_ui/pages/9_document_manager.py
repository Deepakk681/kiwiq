    ...
    with st.spinner("Querying document..."):
        try:
            ...
            if versioned_doc:
                ...
                doc_data = getattr(versioned_doc, 'data', None)
                ...
                st.session_state['current_doc'] = {
                    ...
                    'is_versioned': True,
                    'data': doc_data,
                    'version': getattr(versioned_doc, 'version', None),
                    'created_at': doc_data.get('created_at', None) if isinstance(doc_data, dict) else None,
                    'updated_at': doc_data.get('updated_at', None) if isinstance(doc_data, dict) else None
                }
            else:
                # Try unversioned
                ...
                if unversioned_doc:
                    ...
                    doc_data = getattr(unversioned_doc, 'data', None)
                    # Store in session state for editing
                    st.session_state['current_doc'] = {
                        'namespace': namespace,
                        'docname': docname,
...
def _display_document_details(doc: Union[wf_schemas.CustomerDataRead, wf_schemas.CustomerDataUnversionedRead], is_versioned: bool) -> None:
    ...
    with col1:
        ...
        if isinstance(doc_data, dict):
            ...
            st.code(str(doc_data), language="text")
    
    with col2:
        st.subheader("ℹ️ Metadata")
        
        # Get timestamps from document data
        doc_data = getattr(doc, 'data', None)
        created_at = doc_data.get('created_at') if isinstance(doc_data, dict) else None
        updated_at = doc_data.get('updated_at') if isinstance(doc_data, dict) else None
        ...
        if updated_at:
            try:
                updated_str = updated_at.strftime("%Y-%m-%d %H:%M:%S") if hasattr(updated_at, 'strftime') else str(updated_at)
            except:
            ...
            if documents:
                ...
                if len(documents) > 0:
                    ...
                    for doc in documents:
                        ...
                        if updated_at:
                            try:
                                updated_str = updated_at.strftime("%Y-%m-%d %H:%M") if hasattr(updated_at, 'strftime') else str(updated_at)
                            except:
                                updated_str = str(updated_at)
