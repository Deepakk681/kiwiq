from workflow_service.registry.registry import MockRegistry
from workflow_service.registry.nodes.llm.llm_node import LLMNode

from workflow_service.registry.nodes.llm.prompt import PromptConstructorNode
from workflow_service.registry.nodes.core.dynamic_nodes import InputNode, OutputNode, HITLNode

default_registry = MockRegistry()

default_registry.register_node(LLMNode)
default_registry.register_node(PromptConstructorNode)
default_registry.register_node(InputNode)
default_registry.register_node(OutputNode)
default_registry.register_node(HITLNode)

