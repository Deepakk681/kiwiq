...
class TestReactionCreditsCalculator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_settings = setup_test_settings()
        
        
    def test_zero_limit(self):
        """Test that zero limit returns zero credits."""
        min_credits, max_credits = _calculate_reaction_credits(0, self.mock_settings)
    ...
    def test_none_limit(self):
        ...
        min_credits, max_credits = _calculate_reaction_credits(None, self.mock_settings)
    ...
    def test_single_batch(self):
        ...
        min_credits, max_credits = _calculate_reaction_credits(TEST_REACTION_BATCH_SIZE, self.mock_settings)
        self.assertEqual(min_credits, 1)
        self.assertEqual(max_credits, 1)
    ...
    def test_multiple_batches(self):
        ...
        min_credits, max_credits = _calculate_reaction_credits(limit, self.mock_settings)
...
class TestProfileCreditCalculator(unittest.TestCase):
    ...
    def test_user_profile_credits(self):
        ...
        min_credits, max_credits = calculate_credits(JobType.FETCH_USER_PROFILE, request, self.mock_settings)
        self.assertEqual(min_credits, 1)
        self.assertEqual(max_credits, 1)
    ...
    def test_company_profile_credits(self):
        ...
        min_credits, max_credits = calculate_credits(JobType.FETCH_COMPANY_PROFILE, request, self.mock_settings)
        self.assertEqual(min_credits, 1)
        self.assertEqual(max_credits, 1)
    ...
    def test_wrong_request_type_for_profile(self):
        ...
        with self.assertRaises(TypeError):
            calculate_credits(JobType.FETCH_USER_PROFILE, request, self.mock_settings)
    ...
    def test_wrong_request_type_for_company(self):
        ...
        with self.assertRaises(TypeError):
            calculate_credits(JobType.FETCH_COMPANY_PROFILE, request, self.mock_settings)
    ...
    def test_zero_posts(self):
        ...
        for job_type in self.job_types:
            with self.subTest(job_type=job_type):
                ...
                min_credits, max_credits = calculate_credits(job_type, request, self.mock_settings)
    ...
    def test_wrong_request_type(self):
        ...
        for job_type in self.job_types:
            with self.subTest(job_type=job_type):
                request = ProfileRequest(username="testuser")
                with self.assertRaises(TypeError):
                    calculate_credits(job_type, request, self.mock_settings)
...
class TestUserActivityCreditCalculator(unittest.TestCase):
    ...
    def test_user_comments_activity_credits(self):
        ...
        request = ProfileRequest(username="testuser")
        min_credits, max_credits = calculate_credits(JobType.FETCH_USER_COMMENTS_ACTIVITY, request, self.mock_settings)
        self.assertEqual(min_credits, 1)
        self.assertEqual(max_credits, 1)
    ...
    def test_wrong_request_type(self):
        ...
        request = PostsRequest(username="testuser")
        with self.assertRaises(TypeError):
            calculate_credits(JobType.FETCH_USER_COMMENTS_ACTIVITY, request, self.mock_settings)
...
class TestSinglePostDetailsCreditCalculator(unittest.TestCase):
    ...
    def test_post_reactions_credits(self):
        ...
        min_credits, max_credits = calculate_credits(JobType.FETCH_POST_REACTIONS, request, self.mock_settings)
    ...
    def test_post_comments_profile_credits(self):
        ...
        min_credits, max_credits = calculate_credits(JobType.FETCH_POST_COMMENTS, request, self.mock_settings)
        self.assertEqual(min_credits, 1)
        self.assertEqual(max_credits, 1)
    ...
    def test_wrong_request_type_reactions(self):
        ...
        request = ProfileRequest(username="testuser")
        with self.assertRaises(TypeError):
            calculate_credits(JobType.FETCH_POST_REACTIONS, request, self.mock_settings)
            
    def test_wrong_request_type_comments(self):
        """Test that using the wrong request type for post comments raises TypeError."""
        request = PostsRequest(username="testuser")
        with self.assertRaises(TypeError):
            calculate_credits(JobType.FETCH_POST_COMMENTS, request, self.mock_settings)
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_unknown_job_type(self):
        ...
        mock_job_type.value = "unknown"
        request = ProfileRequest(username="testuser")
        min_credits, max_credits = calculate_credits(mock_job_type, request, self.mock_settings)
